...
public class SpringMvcProtocol extends AbstractProxyProtocol {
    ...
    protected <T> T doRefer(Class<T> type, URL url) throws RpcException {
        ...
        Api apiAno = type.getAnnotation(Api.class);
        if (apiAno != null && void.class != apiAno.fallback() && ClassUtils.isPresent("feign.hystrix.HystrixFeign", SpringMvcProtocol.class.getClassLoader())) {
            T bean = SpringUtil.getBeanNamesForType(apiAno.fallback()).size() > 0 ? (T) SpringUtil.getBean(apiAno.value()) : (T) objectNewInstance(apiAno.fallback());
